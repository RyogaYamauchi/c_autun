...
int WriteTerrian();
int enemyAI(charctor*);
int Damage(charctor*);
int CheckRoom(charctor**);
int CheckRoot(charctor**);
int judgeX(int);
int judgeY(int);
int attack();
...
int enemyCount = 0;
charctor player = {10,10,10,1,1};  //プレイヤーを定義
charctor enemy[10] = {};
int Root[9][4][2]={};
int stepX = 0;
int stepY = 0;
...
int main(void){
  ...
  while(1){  //メイン処理
    ...
    if(enemyCount<5){  //5体まで出現
      enemyCount = AppearEnemy(enemyCount);
    }
    HgLClear(charctorLayer);
    CharctorController(player);    //キャラクターのアクションを操作
    for(i=0;i<enemyCount;i++){     //敵の数だけ敵を描画
      enemyAI(&enemy[i]);
...
int attack(){
  int i,j;
  if(player.moveFlag == 0) {
    for(i=0;i<enemyCount;i++){
      if(terrian[enemy[i].x][enemy[i].y]==terrian[player.x+1][player.y]) enemy[i].hp--;
    }
  }
  if(player.moveFlag == 1) {
    for(i=0;i<enemyCount;i++){
      if(terrian[enemy[i].x][enemy[i].y]==terrian[player.x][player.y+1]) enemy[i].hp--;
    }
  }
  if(player.moveFlag == 2) {
    for(i=0;i<enemyCount;i++){
      if(terrian[enemy[i].x][enemy[i].y]==terrian[player.x-1][player.y]) enemy[i].hp--;
    }
  }
  if(player.moveFlag == 3) {
    for(i=0;i<enemyCount;i++){
      if(terrian[enemy[i].x][enemy[i].y]==terrian[player.x][player.y-1]) enemy[i].hp--;
    }
  }
...
int UIController(){
  ...
  HgWText(UILayer,10,50,"HP : %d/10",hp/10);
  HgWSetFillColor(UILayer,HG_GREEN);
  HgWBoxFill(UILayer,10,30,hp,20,1);
  HgWSetFillColor(UILayer,HG_RED);
  HgWBoxFill(UILayer,110-damage,30,damage,20,1);
