...
854003 山内龍我
  ...
  *操作説明*
    プレイ画面のみ操作可能です
    WASDキーでの上下左右移動になります
    Eキーでプレイヤーが向いている方向に攻撃します
　　プレイヤーの方向は、最後に移動した方向に依存します
　　UI画面のHPが０になったらGAMEOVERです。

　　また、本作品はローグライクダンジョンの生成（システム）に重点を置いているため、
　　GAMEOVERになっても続けることができます、また、クリアも実装しておりません
　　階層が高いのに、Lvが低いとすぐにGAMEOVERになる仕様にはなっています。
　　グラフィック、音楽についても制作してみたものの、ゲームとして全ての要素をつけるのは時間がかかりすぎたので、今回は実装しておりません。
　　基本的には、ダンジョン生成、プレイヤーの移動、Lv、体力、攻撃システム、敵のAI、敵の移動アルゴリズム、UI等々の実装をしました。
　　詳細については、下のコメント欄に全て記載しております。
　　
    地形 : 0
    部屋 : 1
    道  : 2
...
int attackAnimationRight(charctor **charctor){
  ...
  if((*charctor)->Exp >= 1){
    ...
    HgWSetFillColor(gameLayer,HG_WHITE);
    HgWBoxFill(gameLayer,((14-player.x-x)*magnification),((8-player.y-y)*magnification),magnification*2,magnification*2,0);  //プレイヤー
    HgWSetFillColor(animationLayer,HG_RED);
    for(i=0;i<20;i+=2){
      HgLClear(animationLayer);
      HgWCircleFill(animationLayer,((16-player.x-x)*magnification)-20+i,((10-player.y-y)*magnification)-20,charctorSize*2,1);  //プレイヤー
      HgSleep(0.005);
    }
  }
  HgLClear(animationLayer);
...
int enemyAI(charctor *charctor){
  ...
  if(terrian[charctor->x+1][charctor->y]==9 || terrian[charctor->x-1][charctor->y] ==9 || terrian[charctor->x][charctor->y+1]==9||terrian[charctor->x][charctor->y-1]==9){  //周囲にプレイヤーが入れば攻撃
    Damage(&player,charctor->attack);
  }
