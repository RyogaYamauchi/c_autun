...
int main(void){
  ...
  logID = HgWOpen(200,100,1000,200);
  logLayer = logID;
...
int CharctorController(charctor *charctor){
  ...
  if(charctor->hp==0){
    if(charctor->Exp != 0){
      ...
      charctor->hp=-1;
      enemyCount--;
      player.Exp+=(20*level*5);
      strcpy(logStore,"敵を倒した!! 経験値を得た");
      logContoller(logStore);
    }
  }
  return 0;
